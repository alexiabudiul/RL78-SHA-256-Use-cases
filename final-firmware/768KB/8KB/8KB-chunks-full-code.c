...
#define TOTAL_CHUNKS_COUNT   96          
#define CHUNK_SIZE_8KB       (8192UL)    // 8 KB
#define BLOCK_SIZE_8KB       (8192UL)    // 8 KB
...
void process_current_8kb_vector(void) {
    ...
    for(uint32_t offset = 0; offset < CHUNK_SIZE_8KB; offset += BLOCK_SIZE_8KB) {
        volatile uint8_t *current_8kb_ptr = &current_vector_8k[offset];
        sha256_compute((const uint8_t*)current_8kb_ptr, BLOCK_SIZE_8KB, digest);
    }
}
