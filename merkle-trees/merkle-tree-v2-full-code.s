	.text
	.file	"merkle-tree-v2-full-code.c"
...
.Lfunc_end4:
	.size	_sha256_compute, .Lfunc_end4-_sha256_compute
                                        ; -- End function
	.globl	_init_merkle_system             ; -- Begin function init_merkle_system
	.type	_init_merkle_system,@function
_init_merkle_system:                    ; @init_merkle_system
; %bb.0:
	clrb !LOWW(_level_occupied)
	clrb !LOWW(_level_occupied+1)
	clrb !LOWW(_level_occupied+2)
	clrb !LOWW(_level_occupied+3)
	clrb !LOWW(_level_occupied+4)
	clrb !LOWW(_level_occupied+5)
	clrb !LOWW(_level_occupied+6)
	clrb !LOWW(_level_occupied+7)
	clrb !LOWW(_level_occupied+8)
	clrb !LOWW(_level_occupied+9)
	clrb !LOWW(_level_occupied+10)
	clrb !LOWW(_level_occupied+11)
	clrb !LOWW(_level_occupied+12)
	clrb !LOWW(_level_occupied+13)
	clrb !LOWW(_level_occupied+14)
	clrb !LOWW(_level_occupied+15)
	clrb !LOWW(_level_occupied+16)
	clrb !LOWW(_level_occupied+17)
	clrb !LOWW(_level_occupied+18)
	clrb !LOWW(_level_occupied+19)
	clrb !LOWW(_level_occupied+20)
	clrb !LOWW(_level_occupied+21)
	clrb !LOWW(_level_occupied+22)
	clrb !LOWW(_level_occupied+23)
	ret
.Lfunc_end5:
	.size	_init_merkle_system, .Lfunc_end5-_init_merkle_system
                                        ; -- End function
	.globl	_generate_leaf_data             ; -- Begin function generate_leaf_data
	.type	_generate_leaf_data,@function
_generate_leaf_data:                    ; @generate_leaf_data
; %bb.0:
	movw de, ax
	movw hl, #0
...
; %bb.2:
	ret
.Lfunc_end6:
	.size	_generate_leaf_data, .Lfunc_end6-_generate_leaf_data
                                        ; -- End function
	.globl	_add_leaf_to_tree               ; -- Begin function add_leaf_to_tree
	.type	_add_leaf_to_tree,@function
_add_leaf_to_tree:                      ; @add_leaf_to_tree
; %bb.0:
	subw sp, #36
	movw hl, ax
	clrw bc
	movw de, #LOWW(_merkle_stack+15)
	mov a, [hl]
	mov [sp+0], a
	mov a, [hl+1]
	mov [sp+1], a
...
; %bb.3:                                ;   in Loop: Header=BB7_1 Depth=1
	...
	movw hl, ax
	mov a, [hl]
	mov !LOWW(_combine_buffer), a
	movw ax, de
	addw ax, #-14
	movw hl, ax
	mov a, [hl]
	mov !LOWW(_combine_buffer+1), a
	movw ax, de
	addw ax, #-13
	movw hl, ax
	mov a, [hl]
	mov !LOWW(_combine_buffer+2), a
	movw ax, de
	addw ax, #-12
	movw hl, ax
	mov a, [hl]
	mov !LOWW(_combine_buffer+3), a
	movw ax, de
	addw ax, #-11
	movw hl, ax
	mov a, [hl]
	mov !LOWW(_combine_buffer+4), a
	movw ax, de
	addw ax, #-10
	movw hl, ax
	mov a, [hl]
	mov !LOWW(_combine_buffer+5), a
	movw ax, de
	addw ax, #-9
	movw hl, ax
	mov a, [hl]
	mov !LOWW(_combine_buffer+6), a
	movw ax, de
	addw ax, #-8
	movw hl, ax
	mov a, [hl]
	mov !LOWW(_combine_buffer+7), a
	movw ax, de
	addw ax, #-7
	movw hl, ax
	mov a, [hl]
	mov !LOWW(_combine_buffer+8), a
	movw ax, de
	addw ax, #-6
	movw hl, ax
	mov a, [hl]
	mov !LOWW(_combine_buffer+9), a
	movw ax, de
	addw ax, #-5
	movw hl, ax
	mov a, [hl]
	mov !LOWW(_combine_buffer+10), a
	movw ax, de
	addw ax, #-4
	movw hl, ax
	mov a, [hl]
	mov !LOWW(_combine_buffer+11), a
	movw ax, de
	addw ax, #-3
	movw hl, ax
	mov a, [hl]
	mov !LOWW(_combine_buffer+12), a
	movw ax, de
	decw ax
	decw ax
	movw hl, ax
	mov a, [hl]
	mov !LOWW(_combine_buffer+13), a
	push de
	pop hl
	decw hl
	mov a, [hl]
	mov !LOWW(_combine_buffer+14), a
	push de
	pop hl
	mov a, [hl]
	mov !LOWW(_combine_buffer+15), a
	mov a, [hl+1]
	mov !LOWW(_combine_buffer+16), a
	mov a, [hl+2]
	mov !LOWW(_combine_buffer+17), a
	mov a, [hl+3]
	mov !LOWW(_combine_buffer+18), a
	mov a, [hl+4]
	mov !LOWW(_combine_buffer+19), a
	mov a, [hl+5]
	mov !LOWW(_combine_buffer+20), a
	mov a, [hl+6]
	mov !LOWW(_combine_buffer+21), a
	mov a, [hl+7]
	mov !LOWW(_combine_buffer+22), a
	mov a, [hl+8]
	mov !LOWW(_combine_buffer+23), a
	mov a, [hl+9]
	mov !LOWW(_combine_buffer+24), a
	mov a, [hl+10]
	mov !LOWW(_combine_buffer+25), a
	mov a, [hl+11]
	mov !LOWW(_combine_buffer+26), a
	mov a, [hl+12]
	mov !LOWW(_combine_buffer+27), a
	mov a, [hl+13]
	mov !LOWW(_combine_buffer+28), a
	mov a, [hl+14]
	mov !LOWW(_combine_buffer+29), a
	mov a, [hl+15]
	mov !LOWW(_combine_buffer+30), a
	mov a, [hl+16]
	mov !LOWW(_combine_buffer+31), a
	mov a, [sp+0]
	mov !LOWW(_combine_buffer+32), a
	mov a, [sp+1]
	mov !LOWW(_combine_buffer+33), a
	mov a, [sp+2]
	mov !LOWW(_combine_buffer+34), a
	mov a, [sp+3]
	mov !LOWW(_combine_buffer+35), a
	mov a, [sp+4]
	mov !LOWW(_combine_buffer+36), a
	mov a, [sp+5]
	mov !LOWW(_combine_buffer+37), a
	mov a, [sp+6]
	mov !LOWW(_combine_buffer+38), a
	mov a, [sp+7]
	mov !LOWW(_combine_buffer+39), a
	mov a, [sp+8]
	mov !LOWW(_combine_buffer+40), a
	mov a, [sp+9]
	mov !LOWW(_combine_buffer+41), a
	mov a, [sp+10]
	mov !LOWW(_combine_buffer+42), a
	mov a, [sp+11]
	mov !LOWW(_combine_buffer+43), a
	mov a, [sp+12]
	mov !LOWW(_combine_buffer+44), a
	mov a, [sp+13]
	mov !LOWW(_combine_buffer+45), a
	mov a, [sp+14]
	mov !LOWW(_combine_buffer+46), a
	mov a, [sp+15]
	mov !LOWW(_combine_buffer+47), a
	mov a, [sp+16]
	mov !LOWW(_combine_buffer+48), a
	mov a, [sp+17]
	mov !LOWW(_combine_buffer+49), a
	mov a, [sp+18]
	mov !LOWW(_combine_buffer+50), a
	mov a, [sp+19]
	mov !LOWW(_combine_buffer+51), a
	mov a, [sp+20]
	mov !LOWW(_combine_buffer+52), a
	mov a, [sp+21]
	mov !LOWW(_combine_buffer+53), a
	mov a, [sp+22]
	mov !LOWW(_combine_buffer+54), a
	mov a, [sp+23]
	mov !LOWW(_combine_buffer+55), a
	mov a, [sp+24]
	mov !LOWW(_combine_buffer+56), a
	mov a, [sp+25]
	mov !LOWW(_combine_buffer+57), a
	mov a, [sp+26]
	mov !LOWW(_combine_buffer+58), a
	mov a, [sp+27]
	mov !LOWW(_combine_buffer+59), a
	mov a, [sp+28]
	mov !LOWW(_combine_buffer+60), a
	mov a, [sp+29]
	mov !LOWW(_combine_buffer+61), a
	mov a, [sp+30]
	mov !LOWW(_combine_buffer+62), a
	mov a, [sp+31]
	mov !LOWW(_combine_buffer+63), a
	movw ax, bc
	movw [sp+32], ax
	movw ax, de
	movw [sp+34], ax
	...
	movw ax, sp
	movw de, ax
	movw ax, #LOWW(_combine_buffer)
	call !_sha256_compute
	movw ax, [sp+32]
	movw bc, ax
	movw ax, [sp+34]
	mov LOWW(_level_occupied)[bc], #0
	incw bc
	addw ax, #32
	movw de, ax
	movw ax, bc
...
; %bb.4:
	mov a, [sp+0]
	mov !LOWW(_merkle_stack+736), a
	mov a, [sp+1]
	mov !LOWW(_merkle_stack+737), a
	mov a, [sp+2]
	mov !LOWW(_merkle_stack+738), a
	mov a, [sp+3]
	mov !LOWW(_merkle_stack+739), a
	mov a, [sp+4]
	mov !LOWW(_merkle_stack+740), a
	mov a, [sp+5]
	mov !LOWW(_merkle_stack+741), a
	mov a, [sp+6]
	mov !LOWW(_merkle_stack+742), a
	mov a, [sp+7]
	mov !LOWW(_merkle_stack+743), a
	mov a, [sp+8]
	mov !LOWW(_merkle_stack+744), a
	mov a, [sp+9]
	mov !LOWW(_merkle_stack+745), a
	mov a, [sp+10]
	mov !LOWW(_merkle_stack+746), a
	mov a, [sp+11]
	mov !LOWW(_merkle_stack+747), a
	mov a, [sp+12]
	mov !LOWW(_merkle_stack+748), a
	mov a, [sp+13]
	mov !LOWW(_merkle_stack+749), a
	mov a, [sp+14]
	mov !LOWW(_merkle_stack+750), a
	mov a, [sp+15]
	mov !LOWW(_merkle_stack+751), a
	mov a, [sp+16]
	mov !LOWW(_merkle_stack+752), a
	mov a, [sp+17]
	mov !LOWW(_merkle_stack+753), a
	mov a, [sp+18]
	mov !LOWW(_merkle_stack+754), a
	mov a, [sp+19]
	mov !LOWW(_merkle_stack+755), a
	mov a, [sp+20]
	mov !LOWW(_merkle_stack+756), a
	mov a, [sp+21]
	mov !LOWW(_merkle_stack+757), a
	mov a, [sp+22]
	mov !LOWW(_merkle_stack+758), a
	mov a, [sp+23]
	mov !LOWW(_merkle_stack+759), a
	mov a, [sp+24]
	mov !LOWW(_merkle_stack+760), a
	mov a, [sp+25]
	mov !LOWW(_merkle_stack+761), a
	mov a, [sp+26]
	mov !LOWW(_merkle_stack+762), a
	mov a, [sp+27]
	mov !LOWW(_merkle_stack+763), a
	mov a, [sp+28]
	mov !LOWW(_merkle_stack+764), a
	mov a, [sp+29]
	mov !LOWW(_merkle_stack+765), a
	mov a, [sp+30]
	mov !LOWW(_merkle_stack+766), a
	mov a, [sp+31]
	mov !LOWW(_merkle_stack+767), a
	addw sp, #36
	ret
.LBB7_2:
	movw ax, de
	...
	mov a, [sp+31]
	mov [hl+16], a
	mov LOWW(_level_occupied)[bc], #1
	addw sp, #36
	ret
.Lfunc_end7:
	.size	_add_leaf_to_tree, .Lfunc_end7-_add_leaf_to_tree
                                        ; -- End function
	.globl	_run_merkle_benchmark           ; -- Begin function run_merkle_benchmark
	.type	_run_merkle_benchmark,@function
_run_merkle_benchmark:                  ; @run_merkle_benchmark
; %bb.0:
	subw sp, #38
	clrw bc
	clrb !LOWW(_level_occupied)
...
; %bb.3:                                ;   in Loop: Header=BB8_1 Depth=1
	...
	movw ax, hl
	movw [sp+34], ax
	movw ax, #LOWW(_leaf_buffer)
	call !_sha256_compute
	movw ax, [sp+36]
	call !_add_leaf_to_tree
	movw ax, [sp+34]
	movw hl, ax
	movw ax, hl
	addw ax, #1
...
.LBB8_6:
	...
	ret
.Lfunc_end8:
	.size	_run_merkle_benchmark, .Lfunc_end8-_run_merkle_benchmark
                                        ; -- End function
...
_g_sha_state:
	...
	.type	_level_occupied,@object         ; @level_occupied
	.globl	_level_occupied
_level_occupied:
	.skip	24
	.size	_level_occupied, 24

	.type	_leaf_buffer,@object            ; @leaf_buffer
	.globl	_leaf_buffer
_leaf_buffer:
	.skip	64
	.size	_leaf_buffer, 64

	.type	_merkle_stack,@object           ; @merkle_stack
	.globl	_merkle_stack
_merkle_stack:
	.skip	768
	.size	_merkle_stack, 768

	.type	_combine_buffer,@object         ; @combine_buffer
	.globl	_combine_buffer
_combine_buffer:
	.skip	64
	.size	_combine_buffer, 64
...
_K:
	...
	.addrsig_sym _level_occupied
	.addrsig_sym _leaf_buffer
	.addrsig_sym _merkle_stack
	.addrsig_sym _combine_buffer
