...
/* ============================================================
 * OTA BENCHMARK CONFIGURATION
 * ============================================================ */
#define OTA_BATCH_SIZE    512               // One batch of 512 bytes
#define TOTAL_BATCHES     2048              // 2048 * 512 = 1,048,576 bytes (1 MB)

/* We use only 512 bytes of RAM, avoiding the 32KB limitation! */
uint8_t ota_buffer[OTA_BATCH_SIZE];

/**
 * Function that fills the buffer with "unique" data at each call.
 * Uses the packet index to generate different values.
 */
void generate_unique_batch(uint32_t batch_index) {
    for (uint16_t j = 0; j < OTA_BATCH_SIZE; j++) {
        /* Combine the batch index with the byte index (XOR)
         * The result will be unique for each of the 2048 packets */
        ota_buffer[j] = (uint8_t)((batch_index ^ j) & 0xFF);
    }
}

void run_ota_benchmark(void) {
    uint8_t digest[32];
    uint32_t i; /* Use uint32_t to safely count beyond 32767 */

    for (i = 0; i < TOTAL_BATCHES; i++) {
        /* 1. Generate new data in the buffer (simulates receiving a new OTA packet) */
        generate_unique_batch(i);

        /* 2. Compute SHA-256 for these 512 bytes.
         * This function internally calls sha256_init, update, and final. */
        sha256_compute(ota_buffer, OTA_BATCH_SIZE, digest);
        
        /* At each step "i", the buffer was different, therefore 
         * each "digest" will be unique. */
    }
}
