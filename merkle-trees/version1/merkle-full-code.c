...
#define MERKLE_NODE_INPUT_SIZE  64

#define TEST_ITERATIONS         100000

volatile uint8_t g_merkle_buffer[MERKLE_NODE_INPUT_SIZE];

volatile uint8_t g_dummy_sink;

void init_merkle_data(void) {
    for(int i = 0; i < MERKLE_NODE_INPUT_SIZE; i++) {
        g_merkle_buffer[i] = (uint8_t)(i ^ 0xCC);
    }
}

void run_merkle_hashing(void) {
    uint8_t digest[32];
    uint32_t i;
    for(i = 0; i < TEST_ITERATIONS; i++) {
        sha256_compute((const uint8_t*)g_merkle_buffer, MERKLE_NODE_INPUT_SIZE, digest);
        
        if (i % 10000 == 0) {
            printf("Progress: %lu / %lu completed...\r", (unsigned long)i, TEST_ITERATIONS);
        }
    }
}
