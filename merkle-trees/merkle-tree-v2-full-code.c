 ...
 */
#define TREE_HEIGHT      23          // Change this for 20, 21, 22
 ...
 * BENCHMARK DRIVER
 * ============================================================ */
void run_merkle_benchmark(void) {
    uint8_t leaf_hash[HASH_SIZE];
    uint32_t i;

    init_merkle_system();

    /* Iterate through all leaves (1M, 2M, 4M) */
    for(i = 0; i < TOTAL_LEAVES; i++) {
        
        // 1. Generate transaction data (Overhead)
        generate_leaf_data(i);

        // 2. Compute leaf hash (Workload 1)
        sha256_compute(leaf_buffer, 64, leaf_hash);

        // 3. Insert into tree (Workload 2 - Logic + Internal Hashes)
        add_leaf_to_tree(leaf_hash);
    }
}

int main(void) {
    run_merkle_benchmark();
    return 0;
}