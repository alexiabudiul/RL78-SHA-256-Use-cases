...
/* ============================================================
 * HMAC BENCHMARK CONFIGURATION
 * ============================================================ */
#define HMAC_KEY_SIZE     32                // Cheie de 256 biți
#define HMAC_BLOCK_SIZE   64                // Blocul SHA-256 este 64 bytes
#define SENSOR_DATA_SIZE  64                // Simulăm un pachet de date de 64 bytes
#define BENCHMARK_ITERS   172800           // Simulăm numarul de repetitii dintr o zi, deci 172,800 repetitii

/* Cheia secretă (Hardcoded pentru test) */
...
/* Buffer pentru datele de la "senzori" */
uint8_t sensor_buffer[SENSOR_DATA_SIZE];

/* Buffere pentru padding (necesare pentru algoritmul HMAC) */
                 ...
                 uint8_t output[32])
{
    ...
    /* 1. Pregătire cheie și padding (XOR) 
     * Această parte se poate precalcula în realitate, dar o includem 
     * pentru a măsura efortul complet al algoritmului. 
     */
    
    // Inițializăm pad-urile cu zero
    for(i = 0; i < HMAC_BLOCK_SIZE; i++) {
        k_ipad[i] = 0;
        k_opad[i] = 0;
    }
    
    // Copiem cheia în pad-uri
    if (key_len > HMAC_BLOCK_SIZE) {
        // Dacă cheia e prea lungă, se face hash la ea (nu e cazul nostru aici)
    } else {
        for(i = 0; i < key_len; i++) {
            k_ipad[i] = key[i];
            k_opad[i] = key[i];
    ...
    // XOR cu constantele specifice (0x36 și 0x5C)
    for (i = 0; i < HMAC_BLOCK_SIZE; i++) {
        k_ipad[i] ^= 0x36;
        k_opad[i] ^= 0x5C;
    }

    /* 2. INNER HASH: SHA256(K_ipad || message) */
    sha256_init();
    sha256_update(k_ipad, HMAC_BLOCK_SIZE);  // Hash la cheia modificată
    sha256_update(data, data_len);           // Hash la mesaj
    sha256_final(inner_hash);                // Rezultatul intermediar

    /* 3. OUTER HASH: SHA256(K_opad || inner_hash) */
    sha256_init();
    sha256_update(k_opad, HMAC_BLOCK_SIZE);  // Hash la cheia exterioară
    sha256_update(inner_hash, 32);           // Hash la rezultatul anterior
    sha256_final(output);                    // Rezultatul final (HMAC-ul)
}

/* ============================================================
 * BENCHMARK DRIVER
 * ============================================================ */
void run_hmac_benchmark(void) {
    uint8_t mac_result[32];
    uint32_t i;
    uint8_t j;

    /* Bucla de simulare a traficului zilnic (scaled down) */
    for (i = 0; i < BENCHMARK_ITERS; i++) {
        
        /* A. Generare date "Senzor" (simulăm schimbarea valorilor) */
        for(j = 0; j < SENSOR_DATA_SIZE; j++) {
            sensor_buffer[j] = (uint8_t)((i + j) & 0xFF);
        }

        /* B. Calcul HMAC pentru pachetul curent */
        hmac_sha256(secret_key, HMAC_KEY_SIZE, 
                    sensor_buffer, SENSOR_DATA_SIZE, 
                    mac_result);
                    
        /* La finalul acestui apel, 'mac_result' conține semnătura
         * pe care dispozitivul ar trimite-o prin radio. 
         */
    }
}
